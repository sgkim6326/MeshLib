...
{
    public class IncircleUpscaleSolver: IUpscaleSolver
    {
        public GeometryInfo geometry;
        public double thresRadius;

        public IncircleUpscaleSolver(GeometryInfo geometry, double thres)
        {
            this.geometry = geometry;
            this.thresRadius = thres;
        ...
        public async Task<GeometryInfo> AsyncUpscale()
        {
            ...
            Debug.Log(string.Format("Incircular Upscale: complete\n{0}", result.ToString()));
            return result;
        }
        async Task<GeometryInfo> ComputeUpscale()
        {
            Debug.Log("Incircular Upscale: compute upscale");
            ...
            await Task.Run(() =>
            {
                while (remainedFaces.Count != 0 )
                {
                    ...
                    var tri_area = CalculateFaceArea(totalVertices, cFace);
                    var tri_perimeter = CalculatePerimeter(totalVertices, cFace);
                    var incir_radius = 2 * tri_area / tri_perimeter;
                    if (incir_radius < thresRadius)
                    {
        ...
        {
            Debug.Log("Incircular Upscale: remove duplication");
        ...
        double CalculatePerimeter(List<Vector3> vertices, Face face)
        {
        ...
        double CalculateFaceArea(List<Vector3> vertices, Face face)
        {
            Vector3 p0 = vertices[face.VertexIndex0];
            Vector3 p1 = vertices[face.VertexIndex1];
            Vector3 p2 = vertices[face.VertexIndex2];
